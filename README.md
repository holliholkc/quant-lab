...
* **`quantlab.strategies`** — учебные стратегии-примеры (SMA crossover,
  RSI mean-reversion, Bollinger breakout), связывающие первые два слоя.
...
![Галерея индикаторов](examples/output/indicators_gallery.png)
...
Две вкладки: **«Индикаторы»** — цена с оверлеями (SMA/EMA/Bollinger) и
сменной панелью осциллятора, все параметры — живые ползунки;
**«Бэктест»** — выбираешь стратегию, крутишь параметры и издержки,
смотришь, как эквити сражается с buy-and-hold в реальном времени.
Демка честно комментирует результат: даже если ползунками удалось
обогнать Buy & Hold — это один актив, один период и подбор параметров
задним числом.

## Тесты

34 теста, ключевой принцип — **каждая векторизованная реализация
...
**2. Установить библиотеку со всеми зависимостями.**

```bash
pip install -e ".[dev]"
```

Эта команда ставит сам пакет `quantlab` плюс всё, что нужно для
тестов, примеров и демки: **pytest, pandas, matplotlib, streamlit**.
Если нужно только ядро (без демки и тестов) — `pip install -e .`,
оно тянет один NumPy.

**3. Запускать.**

```bash
pytest                                  # тесты (34 passed)
python examples/run_strategies.py      # таблица и график выше
python examples/plot_indicators.py     # галерея индикаторов
python benchmarks/bench_indicators.py  # бенчмарки
streamlit run app/streamlit_app.py     # интерактивная демка
```

Если `streamlit` «не распознан как команда» — шаг 2 был выполнен без
`[dev]` или в другом окружении; повтори `pip install -e ".[dev]"` при
активированном venv.
